...
@ActiveProfiles("test")
class MemberShipSignupTest extends AbstractWebIntegrationTest {

  @MockitoBean private MailSender mailSender;

  @Test
  void memberShipSingup_happyPath_returns204_andSendsMail() throws Exception {
    ClubInserted club = utilTest.insertClub();
    MockMultipartFile signUp =
        new MockMultipartFile("signUp", "signup.pdf", "application/pdf", "signup".getBytes());
    MockMultipartFile receipt =
        new MockMultipartFile("receipt", "receipt.pdf", "application/pdf", "receipt".getBytes());

    mockMvc
        .perform(
            multipart("/clubs/{club}/contact/memberShipRequest", club.slug())
                .file(signUp)
                .file(receipt))
    ...
    assertThat(mailCaptor.getValue().type()).isEqualTo(MailType.MEMBERSHIP_SIGNUP);
    assertThat(attachmentCaptor.getValue()).hasSize(2);
  }
}
